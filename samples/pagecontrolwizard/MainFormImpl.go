...
func (f *TMainForm) OnActPagePrevExecute(sender lcl.IObject) {
	f.PageControl1.SetActivePageIndex(f.PageControl1.ActivePageIndex() - 1)
	//f.setPage(-1)
}

func (f *TMainForm) OnActPagePrevUpdate(sender lcl.IObject) {
	lcl.AsAction(sender).SetEnabled(f.PageControl1.ActivePageIndex() > 0)
}

func (f *TMainForm) OnActPageNextExecute(sender lcl.IObject) {
	f.PageControl1.SetActivePageIndex(f.PageControl1.ActivePageIndex() + 1)
	//f.setPage(1)
}

func (f *TMainForm) OnActPageNextUpdate(sender lcl.IObject) {
	lcl.AsAction(sender).SetEnabled(f.PageControl1.ActivePageIndex() < f.PageControl1.PageCount()-1)
}
