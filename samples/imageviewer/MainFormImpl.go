...
type TMainFormFields struct {
	...
	orgTitle       string // 保存
	//imgBuffer      *lcl.TPicture
	imgThumb      *lcl.TBitmap
...
func (f *TMainForm) OnFormCreate(sender lcl.IObject) {
	...
	//imgBuffer = lcl.NewPicture()
	//imgBuffer.SetOnChange(f.OnPicChanged)
	f.imgThumb = lcl.NewBitmap()
	//f.setPBPreViewPos()
	// 调试下，先显示
	//f.PBPreview.Show()
...
func (f *TMainForm) OnPicChanged(sender lcl.IObject) {
	fmt.Println("图片改变")
}

func (f *TMainForm) OnFormMouseWheel(sender lcl.IObject, shift types.TShiftState, wheelDelta, x, y int32, handled *bool) {
	if wheelDelta > 0 {
		f.zoom(1)
	} else {
		f.zoom(-1)
...
func (f *TMainForm) zoom(val int) {
	...
	img := f.ImgViewer
	scale := 1.1
	mPos := f.ScreenToClient(lcl.Mouse.CursorPos())
...
func (f *TMainForm) OnFormResize(sender lcl.IObject) {
	// 首次加载后居中
	if f.isAutoCenter {
...
func (f *TMainForm) OnBtnMinClick(sender lcl.IObject) {
	f.setWindowState(types.WsMinimized)
}

func (f *TMainForm) OnBtnCloseClick(sender lcl.IObject) {
	f.Close()
}

func (f *TMainForm) OnBtnMaxClick(sender lcl.IObject) {
	f.setWindowState(types.WsMaximized)
}

func (f *TMainForm) OnBtnRestoreClick(sender lcl.IObject) {
	f.setWindowState(types.WsNormal)
}

func (f *TMainForm) OnBtnMenuClick(sender lcl.IObject) {
	lcl.ShowMessage("别点了，点了我我也不干活！")
}

func (f *TMainForm) OnLblCaptionMouseUp(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
	f.isMouseDown = false
}
...
func (f *TMainForm) OnLblCaptionMouseMove(sender lcl.IObject, shift types.TShiftState, x, y int32) {
	if f.isMouseDown {
		if f.isMax() {
			f.setWindowState(types.WsNormal)
			// 这里做，主要是修复还原后鼠标位置
			ratio := float64(f.Width()) / float64(lcl.Screen.Width())
...
func (f *TMainForm) OnLblCaptionDblClick(sender lcl.IObject) {
...
func (f *TMainForm) OnImgViewerMouseDown(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
	if button == types.MbLeft {
		f.isMouseDown = true
		f.mouseDownPos.X = x
		f.mouseDownPos.Y = y
...
func (f *TMainForm) OnImgViewerMouseUp(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
	f.isMouseDown = false
}

func (f *TMainForm) OnImgViewerMouseMove(sender lcl.IObject, shift types.TShiftState, x, y int32) {
	f.imgMousePos.X = x
	f.imgMousePos.Y = y
	if f.isMouseDown {
...
func (f *TMainForm) OnPBPreviewMouseDown(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {

}

func (f *TMainForm) OnPBPreviewPaint(sender lcl.IObject) {
...
func (f *TMainForm) OnPBPreviewMouseMove(sender lcl.IObject, shift types.TShiftState, x, y int32) {

}

func (f *TMainForm) OnPBPreviewMouseUp(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
...
func (f *TMainForm) OnPnlClientMouseMove(sender lcl.IObject, shift types.TShiftState, x, y int32) {
...
func (f *TMainForm) OnBtnNextClick(sender lcl.IObject) {
	if len(f.curDirImages) == 0 {
		return
	}
...
func (f *TMainForm) OnFormConstrainedResize(sender lcl.IObject, minWidth, minHeight, maxWidth, maxHeight *int32) {
	if runtime.GOOS == "windows" {
		if f.WindowState() == types.WsMaximized {
			*maxHeight = lcl.Screen.WorkAreaHeight()
		}
	}
	*minWidth = 400
	*minHeight = 400
