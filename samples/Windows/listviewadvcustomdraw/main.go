...
type TMainFrom struct {
	*lcl.TForm
	ListView *lcl.TListView
	Icons    *lcl.TImageList
	tempIco  *lcl.TIcon
}
...
func (f *TMainFrom) OnListView1AdvancedCustomDrawSubItem(sender *lcl.TListView, item *lcl.TListItem, subItem int32, state types.TCustomDrawState, stage types.TCustomDrawStage, defaultDraw *bool) {
	if len(tempData) == 0 {
		return
	}
