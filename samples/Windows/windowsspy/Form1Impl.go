...
func (f *TForm1) OnFormCreate(sender lcl.IObject) {
	f.SetLeft(lcl.Screen.Width() - f.Width() - 5)
	f.SetTop(lcl.Screen.WorkAreaHeight() - f.Height() - 30)
	lcl.Screen.SetCursors(1, f.Img3.Picture().Icon().Handle())
}
...
func (f *TForm1) OnImg1MouseDown(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
	if shift.In(types.SsLeft) {
		f.lasthWnd = 0 //win.WindowFromPoint(lcl.Mouse.CursorPos())
		lcl.Screen.SetCursor(types.TCursor(1))
		f.Img2.SetVisible(true)
		f.Img1.SetVisible(false)
	}
}

func (f *TForm1) OnImg1MouseUp(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
	lcl.Screen.SetCursor(types.CrDefault)
...
func (f *TForm1) OnImg1MouseMove(sender lcl.IObject, shift types.TShiftState, x, y int32) {
	if shift.In(types.SsLeft) {
...
func (f *TForm1) OnChkonTopClick(sender lcl.IObject) {
...
func (f *TForm1) OnChkSwitchHexClick(sender lcl.IObject) {
