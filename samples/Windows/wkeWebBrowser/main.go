...
import (
	...
	"github.com/energye/golcl/pkgs/wke"
)
...
func main() {
	...
	// 返回按钮
	btnBack := lcl.NewButton(mainForm)
	btnBack.SetParent(lPnl)
	btnBack.SetCaption("<")
	btnBack.SetHint("后退")
	btnBack.SetBounds(5, (lPnl.Height()-30)/2, 30, 30)
	btnBack.SetOnClick(func(sender lcl.IObject) {
		wkeBrw.GoBack()
	})

	// 前进按钮
	btnForward := lcl.NewButton(mainForm)
	...
	btnForward.SetOnClick(func(sender lcl.IObject) {
		wkeBrw.GoForward()
	})

	// 重载按钮
	btnReload := lcl.NewButton(mainForm)
	...
	btnReload.SetOnClick(func(sender lcl.IObject) {
		wkeBrw.Reload()
	})

	// 中间地址栏
	cPnl := lcl.NewPanel(mainForm)
	cPnl.SetParent(pnl)
	...
	btnGo.SetOnClick(func(sender lcl.IObject) {
		if edit.Text() != "" {
	...
	pnl.SetOnResize(func(sender lcl.IObject) {
		if wkeBrw != nil {
	...
	mainForm.SetOnKeyPress(func(sender lcl.IObject, key *types.Char) {
		if *key == '\r' {
			btnGo.Click()
		}
	})
	...
	mainForm.SetOnDestroy(func(lcl.IObject) {
		if wkeBrw.IsVaild() {
			wkeBrw.Free()
		}
	})
	if wkeBrw.IsVaild() {
		...
		refWebbrowserTimer := lcl.NewTimer(mainForm)
		refWebbrowserTimer.SetInterval(80)
		refWebbrowserTimer.SetEnabled(true)
		refWebbrowserTimer.SetOnTimer(func(sender lcl.IObject) {
