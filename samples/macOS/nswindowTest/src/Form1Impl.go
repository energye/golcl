...
func (f *TForm1) OnFormShow(sender lcl.IObject) {

}

func (f *TForm1) OnFormResize(sender lcl.IObject) {
	// 应该是监视最大化和还原，
	C.resizeAction(f.nsWinPtr)
}

func (f *TForm1) onCenterAlign(sender *lcl.TWinControl, control *lcl.TControl, newLeft, newTop, newWidth, newHeight *int32, alignRect *types.TRect, alignInfo types.TAlignInfo) {
...
func (f *TForm1) isFullScreen() bool {
	return f.Width() == lcl.Screen.Width() && f.Height() == lcl.Screen.Height()
}

func (f *TForm1) OnPanel1DblClick(sender lcl.IObject) {
	// 全屏了
	if f.isFullScreen() {
		return
	}
...
func (f *TForm1) OnPanel1MouseMove(sender lcl.IObject, shift types.TShiftState, x, y int32) {
	if f.isDown {
...
func (f *TForm1) OnPanel1MouseUp(sender lcl.IObject, button types.TMouseButton, shift types.TShiftState, x, y int32) {
	f.isDown = false
}
